    ...
    if (sd) {
        ...
        p_dens_plot +
            geom_area(data = sd_shaded[[2]] %>%
                     filter(between(x, sd_out[[1]], sd_out[[2]])),
                      aes(x = x, y = y),
                      fill = "blue",
...
## mt_split <- mtcars %>% split(between(.$mpg, 15, 20))
...
```{r}
sd_pos <- mean + (2 * sd(portfolio_tq$returns))
sd_neg <- mean - (2 * sd(portfolio_tq$returns))

sd_pos_shaded <- ggplot_build(port)
```
