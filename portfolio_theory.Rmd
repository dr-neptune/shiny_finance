...
<!-- ```{r} -->
<!-- server <- function(input, output, session) { -->
<!--     portfolio_tq <- eventReactive(input$go, { -->
<!--         syms <- c(input$stock_SPY, input$stock_EFA, -->
<!--                   input$stock_IJS, input$stock_EEM, -->
<!--                   input$stock_AGG) -->

<!--         prices <- get_prices(syms, input$date) -->

<!--         weights <- c(input$wt_1 / 100, input$wt_2 / 100, -->
<!--                      input$wt_3 / 100, input$wt_4 / 100, -->
<!--                      input$wt_5 / 100) -->
        
<!--         # convert returns to a tibble -->
<!--         portfolio_monthly <- prices() %>% -->
...
<!--     # calculate market return and convert it to a tibble -->
<!--     market_return <- eventReactive(input$go, { -->
<!--         market_return <- getSymbols("SPY", -->
<!--                                     src = "yahoo", -->
<!--                                     from = input$date, -->
<!--                                     auto.assign = TRUE, -->
<!--                                     warnings = FALSE) %>% -->
...
<!--     # run regression and get beta -->
<!--     beta_dplyr <- eventReactive(input$go, { -->
<!--         beta_dplyr <- portfolio_monthly() %>% -->
<!--             do(model = lm(returns ~ market_return()$returns, data = .))}) -->
    
<!--     # create augmented model for highcharter -->
<!--     portfolio_augmented <- eventReactive(input$go, { -->
<!--         beta_dplyr() %>% -->
<!--             augment(model) %>% -->
<!--             rename(mkt_rtns = market_return.returns) %>% -->
<!--             select(returns, mkt_rtns, .fitted) %>% -->
<!--             mutate(date = portfolio_monthly()$date)}) -->

<!--     output$hchart_plot <- renderHighchart({ -->
<!--         highchart() %>% -->
<!--             hc_title(text = "Scatter with Regression") %>% -->
<!--             hc_add_series(portfolio_augmented(), -->
<!--                           type = "scatter", -->
<!--                           color = "cornflowerblue", -->
<!--                           hcaes(x = round(mkt_rtns, 4), -->
...
<!--             hc_yAxis(title = list(text = "Market Returns")) %>% -->
<!--             hc_yAxis(title = list(text = "Portfolio Returns")) %>% -->
<!--             hc_add_theme(hc_theme_flat())}) -->
...
<!--         beta_dplyr() %>% -->
<!--             tidy(model) %>% -->
<!--             mutate(term = c("alpha", "beta"))}, digits = 4) -->
<!-- } -->
<!-- ``` -->
...
<!--         get_returns(syms, symbols, input$date)}) -->
...
<!--     # calculate market return and convert to a tibble -->
<!--     market_return <- eventReactive(input$go, { -->
<!--       market_return <- getSymbols("SPY", -->
<!--                                   src = "yahoo", -->
<!--                                   from = input$date, -->
...
<!-- get_prices("SPY", "2012-12-31") %>% -->
<!--     get_tq_returns(1) %>% -->
<!--     .[["returns"]] -> market_returns_example -->

<!-- get_prices(c("EFA", "IJS"), -->
<!--            from_date = "2012-12-31") %>% -->
<!--     get_tq_returns(weights = c(0.5, 0.5)) -> portfolio_returns_example -->
...
<!-- augment_returns(portfolio_returns_example, -->
<!--                 market_returns_example) -->

<!-- alpha_beta_table <- function(data, m_returns) { -->
<!--         data %>% -->
<!--             do(model = lm(returns ~ m_returns, -->
<!--                           data = .)) %>% -->
<!--             tidy(model) %>% -->
<!--             mutate(term = c("alpha", "beta")) -->
<!-- } -->

<!-- alpha_beta_table(portfolio_returns_example, -->
<!--                  market_returns_example) -->
...
<!--         portfolio_returns() %>% -->
<!--             alpha_beta_table(m_returns = market_returns())}, -->
<!--         digits = 4) -->
<!-- } -->
<!-- ``` -->
