...
augment_returns <- function(p_data, m_returns) {
    lmod <- lm(returns ~ m_returns, data = data)
...
lmod <- lm(returns ~ market_returns_example, portfolio_returns_example)

augment(lmod) %>%
    rename("hi" = 2)

portfolio_returns_example[["date"]]
...
server <- function(input, output, session) {
    ...
    beta <- eventReactive(input$go, {
        portfolio_returns() %>%
            augment_returns(m_returns = market_returns())})
